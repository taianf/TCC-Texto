...
second experiment unless station local clocks were synchronized to each other.
...
too imprecise to be taken as a measurement of $L_{ativ}$. This issue will be further discussed 
...
average value observed in the measurements, $\bar{\delta}$. As will be seen later
...
Although each experiment was run for ten minutes, the one-minute time window was found sufficient to illustrate the timing behavior of each platform. During this time interval, the total number of events is 1200 as the arrival
frequency of Ethernet frames at the $eth_0$ network device of station $E_M$ is $20
Hz$. 

We first present in Section \ref{sec:results-1} the results from the first experiment set-up 
regarding the analyzed platforms. Then, in Section \ref{sec:results-2}, we analyze
the procedure suggested by the second experiment. Section \ref{sec:compTable} 
gives a discussion on the differences between these two types of expetiments.
...
Comparing the values of activation latencies of Linux$^{\mathbf{PrtND}}$, 
it can be noticed that the found values are acceptable when compared to
Linux$^{\mathbf{Prt}}$ in the absence of load. 
Nonetheless, the results obtained with system load still indicate a slight
less predictable behavior than Linux$^{\mathbf{Xen}}$.

\subsection{Results from the Second Experiment}
\label{sec:results-2}

As will be seen, the results obtained by the second experiment were similar to those
described in the previous section. In order to avoid repeating the observed patterns
of the analyzed platforms, we summarized the statistics of the results in Table
...
By the second experiment, the measurements are carried out by station $E_T$ (recall
...
for the first experiment. A noticeable difference is a significant increase of the
standard deviation. This can be explained as follows. In the first experiment,
$T_{eth_0}$ issues an interrupt request and finishes its execution. The pending
interrupt is immediately detected and the execution of the associated handler begins
with a minimum delay as the indirection scheme of the Adeos nanokernel guarantees
that no other interruption can delay the start of $T_{PP}$ handler. Such scenarios
...
\multirow{4}{*}{\begin{sideways}\textbf{Set Up 1}\rule{0.1cm}{0.cm}\end{sideways}}
&
...
\hline\hline\hline
%\cline{2-18}
\multirow{4}{*}{\begin{sideways}\textbf{Set Up 2}\rule{0.1cm}{0.cm}\end{sideways}}
&
\vline \rule{0cm}{0.35cm} Mean &
...
&
\vline \rule{0cm}{0.35cm} Max &
 27.3 & 15.6 & 80.6 & 623.3 & 42.2 & 17.3 & 324.4 & 70.5 &
 25.5 & 15.4 & 50.5 & 107.2 &  23.1 & 13.2 & 30.0 & 25.7\\
\cline{2-18}
\cline{2-18}
...
The graph \ref{fig:xenSemSched2} obtained without load deserve a special attention.
...
that explain those results is illustrated by Figure \ref{fig:dispExp3}. When the
...
introducing a constant delay of $10 \mu s$ between the triggering of  $IRQ(T_{PP})$ and
...
As can be seen from the comparison of Table \ref{tab:dataSetUp}, results obtained
with the first set up are corroborated by the second set up. As illustrate with the
...
In this work, we have conducted a timing evaluation of two Linux-based
real-time. The methodology has allowed experimental measurements of interrupt and
activation latencies, in situations of variable load, both of the processor and of
external events processed by interruption.  Two experimental set up were used, the
first based on local measurements was compared with a more regular set up based on
external measurements through the parallel port.  As both experiments gives similar
results, it appears that the first methodology can be efficiently
...
Two experiment set-ups have been defined. The simpler one
 The described experiments considered local measurements (não entendi
...
{\scalebox{0.9}{
\begin{tabular}{@{} p{0.9cm}|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|r|}
\cline{2-17}
\multirow{2}{*}{}
&
\multicolumn{4}{|c|}{\rule{0cm}{0.45cm}Linux$^{\mathbf{Std}}$}&
\multicolumn{4}{|c|}{Linux$^{\mathbf{Prt}}$}&
\multicolumn{4}{|c|}{Linux$^{\mathbf{PrtND}}$}&
\multicolumn{4}{|c|}{Linux$^{\mathbf{Xen}}$}\\
...
&
\rule{0cm}{0.35cm}$L_{irq}$ & $L_{ativ}$ & $L_{irq}$ & $L_{ativ}$ &
$L_{irq}$ & $L_{ativ}$ & $L_{irq}$ & $L_{ativ}$ &
$L_{irq}$ & $L_{ativ}$ & $L_{irq}$ & $L_{ativ}$ &
$L_{irq}$ & $L_{ativ}$ & $L_{irq}$ & $L_{ativ}$\\
\hline
