...
studies can help real-time systems designers choosing the appropriate solution
...
Linux$^{\mathbf{Xen}}$ Xen provides better timing guarantees than
Linux$^{\mathbf{Prt}}$.
...
parts. The first part, referred to as the \textbf{critical section} of the handlers,
runs critical operations immediately after it starts executing and usually requires
that interruptions are disabled.  One may enable interruptions during some parts of
a critical section in order to allow for preemptions. However, such an
implementation must rely on locks to ensure a controlled access to shared data. The
second part of the handler is dedicated to non-critical operations. Its execution
can be delayed and is normally implemented considering that interruptions are
enabled. In Linux, this second part of the handlers are called
\textbf{\emph{softirqs}}.

Clearly, the way interrupt handlers are dealt with by an OS kernel interferes in the system timeliness as a whole. Two metrics, explained next, can be used to analyze the timing behavior of an OS.
...
starting time of the execution of the associated handler is called \textbf{interrupt
  latency}. This latency is one of the parameters that characterizes 
...
while waiting for some event. When this event occurs, the associated interrupt
...
are delayed by the adopted scheduling policy or by other interrupt
requests. Nevertheless, Linux$^{\mathbf{Prt}}$ offers the option \cod{IRQF\_NODELAY}
which disables the threaded implementation of interrupt handlers. With this option,
interrupts are handled as in standard Linux.
...
real-time tasks. Whenever the Linux kernel requests disabling the interrupts, the
nanokernel just makes the Linux kernel believe that interrupts are disabled. The
nanokernel keeps intercepting any hardware interrupts.  The interrupt requests
targeted to Linux are kept enqueued until the Linux kernel requests enabling
interrupts.
...
to the Linux kernel, from which common Linux software libraries are available. At
...
not simple and may require the use of external devices such as oscilloscopes or other
...
To compare the OS platforms two types of experiments were set up, both of which use only
computer stations connected to each other by standard communication devices. These
two experiment types, described in Sections \ref{sec:exp1} and \ref{sec:exp2}, are to
...
During the experiment runs, the measured values of $L_{irq}$ and $L_{ativ}$ were
...
In order to compare the behavior of the analyzed platforms, both latencies can be computed by the described procedure as $L_{irq} = t_2 - t_1$ and $L_{ativ} = t_3 - t_2$, as depicted in Figure \ref{fig:dispExp}. However, it is worth
...
extent such a procedure interfere in the measurements, a second type of experiment was set
up.
...
set-up. Station $E_L$ is configured as before while the other two 
two stations have their roles modified, as illustrated in Figure \ref{fig:config2}.
...
measured correspond to real-time activities executed by $E_M$. However, the
...
The device $eth_0$ is no longer necessary.  In other words, station $E_T$ triggers
PP interrupts at $E_M$ via this parallel port. Station $E_M$ handles such PP
interrupt requests, waking up a real-time task $\tau$ similarly to the previous
experiment. Note that the measurements could not be carried out by $E_M$ in this
second type of experiment unless station local clocks were synchronized to each other.
...
some care must be taken in order to measure $L_{ativ}$ accurately as the
...
mean value observed in the measurements, $\bar{\delta}$. As will be seen later
on, for such a platform $\bar{\delta} = 9 \mu s$ with standard deviation $0.1 \mu
s$. Once this estimation is derived, the operating system patch of $E_T$ must be
fixed so that $\bar{\delta}$ can be used as an accurate estimation of $\delta$
during the experiments.

During the measurements, the obtained values were transfered from memory to
...
$E_M$. When overload were not considered, $E_M$ was set up with its kernel in
...
As for overload scenarios, station $E_M$ was stressed by two different types of
...
maximum allowed rate by the network bandwidth. In other words, more than $100,000$
...
processors and $512 MB$ RAM memory. Three operating system platforms were analyzed, one of which with
...
\item[$\bullet \;$ Linux$^{\mathbf{PrtND}}$:] Linux$^{\mathbf{Prt}}$ with option \cod{IRQF\_NODELAY};
...
not suitable to deal with real-time applications. It also may serve as a 
reference, against which one can compare real-time Linux patches. 
Linux$^{\mathbf{PrtND}}$ corresponds to setting the option
\cod{IRQF\_NODELAY} at the initialization time of PP-IRQ line. Recall that with this option,
interrupt handling of that line is implemented without thread. 
Latencies $L_{irq}$ and $L_{ativ}$ were measured using the
...
running the experiments for ten minutes for each experiment type and platform.

Experimental results are presented through graphs in which the horizontal axes
represent the instant at which the latencies were measured, which ranges from 0 to 60 seconds.
The vertical axes represent the measured latencies in $\mu s$. These values can be multiplied by $2.6 10^3$
to obtain the corresponding number of TSC cycles. 
...
duration of ten minutes of each experiment run. Values outside the vertical axis range
are represented by a triangle near the maximum value.
Although each experiment was run for ten minutes, one-minute time window was found sufficient to illustrate the timing behavior of each platform. During this time interval, the total number of events is 1200 as the arrival
frequency of Ethernet frames at the $eth_0$ network device of station $E_M$ is $20
Hz$. 

We first present in Section \ref{sec:results-1} the results from the first type of experiment.
Then, in Section \ref{sec:results-2}, we analyze
the procedure suggested by the second type of experiment. Section \ref{sec:compTable} 
gives a discussion on the differences between these two types of experiments.
...
Indeed, the maximum value of $L_{ativ}$ was found to be about 17 times the mean value.
...
\subsubsection{Linux$^\mathbf{Prt}$, Linux$^\mathbf{PrtND}$ and Linux$^\mathbf{Xen}$}

Figures \ref{fig:latIrq} and \ref{fig:latAtiv} plot the values of $L_{irq}$ and
$L_{ativ}$, respectively.  Six graphs are shown in each figure. 
...
Indeed, between the instant at which $T_{PP}$ issues the interrupt and
...
load. Although the mean value found for Linux$^{\mathbf{Xen}}$ ($8,7 \mu s$) is
superior to the one found for Linux$^{\mathbf{Prt}}$ ($3,8 \mu s$), the standard deviation is
...
desirable that the worst-case execution time be as close as possible to the average-case execution time.
...
Analyzing the values of activation latencies of Linux$^{\mathbf{PrtND}}$, 
it can be noticed that the found values are acceptable when compared to
Linux$^{\mathbf{Prt}}$ in the absence of load. 
Nonetheless, the results obtained in overload scenarios still indicate a slight
less predictable behavior than Linux$^{\mathbf{Xen}}$.

\subsection{Results from the Second Experiment}
\label{sec:results-2}

As will be seen, the timing patterns obtained by the second experiment were similar to those
described in the previous section. In order to avoid repeating the illustration of such patterns, 
we summarized the statistics of the results in Table
\ref{tab:dataSetUp}, which will be presented in Section \ref{sec:compTable}. Before
...
because: (i) the results of the first type of experiment have indicated that
...
By the experiment set-up, the measurements are carried out by station $E_T$ (recall
...
$E_T$. In other words, since Linux$^{\mathbf{Xen}}$ is being used in both stations,
one expects that $L_{irq} = 2 \delta$ in scenarios without overloads.  This is
exactly what is shown in Figure \ref{fig:xenSem2}, where $2 \delta \approx 18.2$
with standard deviation $0.3$.

Following the above discussion, in order to analyze the behavior of the system in
overload scenarios, one must take into consideration the value of $\delta$ in
$E_T$. Once $\delta$ is subtracted from results plotted in Figure \ref{fig:xenTot2},
it can be seen that the system behaves very similarly to the patterns presented as
for the first type of experiment. A noticeable difference is a significant increase of the
standard deviation. This can be explained as follows. In the first type of experiment,
...
do not occur in the second type of experiment since the parallel port interrupts are
...
second type of experiment captures actual scenarios more accurately as can be seen
...
and Linux$^{\mathbf{Xen}}$ using
 Experiments 1 and 2 \vspace{0.4cm}}
 \label{tab:dataSetUp}
{\scalebox{1}{
...
&&
\rule{0cm}{0.35cm}$\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ & 
$\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ & $\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ &
$\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ & $\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ &
$\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ & $\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$ &
$\mathbf{L_{irq}}$ & $\mathbf{L_{ativ}}$\\
\cline{2-18}
...
The second type of experiment makes it more difficult to determine activation
latencies (recall Figure \ref{fig:dispExp2}).  As mentioned earlier, the interrupt
request issued by $\tau$ may take place before or after $t_2$, turning the value of
$t_3 - t_2$ into an imprecise measurement. For example, if $\tau$ issues an
interrupt request at $E_T$ before $t_2$, there are two pending interrupt requests
when the handler $T_{PP}$ of $E_T$ is activated. Thus, these two requests will be
handled in a row, which makes the value of $t_3- t_2$ too short.  On the other hand,
if the interrupt request by $\tau$ takes place after $t_2$, as represented in Figure
\ref{fig:dispExp2}, $L_{ativ} = t_3 - t_2$ is a much better accurate measurement.

In order to circumvent this measurement problem, an extra and constant delay of $10
\mu s$ was introduced so that the interrupt request issued by $\tau$ always takes
place after $t_2$.  The graphs in Figure \ref{fig:xenSemSched2} show the activation
latencies obtained by this approach. The values are already subtracted from $10 \mu s$ and so
they correspond to the measured values of $L_{ativ}$. As can be seen, the
values in the graphs are very close to the ones obtained by the first type of
experiment as can be seen by the difference between the mean values. 
Also, as expected, the variability is now higher due to the way the experiment was set up. 

%XXX TO BE CONTINUED....
%Doing so, we guaranteed that $E_T$ had time to handle the first interrupt
...
%obtained with set up 1 with a precision better than $0.5 \mu s$.
%The graph  obtained without load deserve a special attention.
%Indeed, the mean value obtained here for the activation latency is much higher
%($6.5$) than this obtained with the first experimental set up ($2.1$).  The scenario
%that explain those results is illustrated by Figure \ref{fig:dispExp3}. 
%When the
%time interval elapsed between the triggering of the first interrupt by $IRQ(T_{PP})$
%and the second interrupt by $IRQ(\tau)$ is leather than $\delta$, the interrupt
%request triggered by $E_M$ arrives at $E_T$ while the first interrupt is still
%pending. Therefore, $E_T$ handles both interrupt in a row, and the latency measured
%is the minimum latency $L_{min}$ that can separate the execution of two consecutive
%interrupt handler.
...
\begin{figure*}[t!]%
 ...
 \caption[Activation latencies]{Linux$^{\mathbf{Xen}}$  latencies on $E_M$ with $20 Hz$ PP-IRQ trigging
   frequency.}
...
for comparing the platform behaviors, as mentioned before. 
In general, the values of $L_{irq}$ obtained by both types of experiment are very close for all platforms, 
which indicates
in the table...
%, are higher when obtained by the second type of experiment. As mentioned before,
%this is due to the 


comAccording to the results in the table, it can be seen that 
Linux$^\mathbf{Xen}$ is the most stable 

As illustrate with the
description of Linux$^{\mathbf{Xen}}$ in the previous section, three significant
differences appears: (i) a constant interrupt delay $\delta$ is introduced by the
second set up regarding the interrupt latency; (ii) the variability of the interrupt
latency measurements is higher in the second experiment; (iii) a possible
interference can occur in the second set up regarding activation latency
measurements.

Another noticeable difference is related to Linux $^{\mathbf{Prt}}$ whose
...
The second set up requires a specific parallel port cable between both station, as
well as a dedicated triggering station able to make the measurements with a good
accuracy. Such a station was obtained here using the Xenomai real-time platform.
Regarding the first set up, a simple connection of a writable pin to the
interruption pin of the parallel port is only needed in order to realize
satisfactory measurements for effect of platform comparison.
...
we noticed some degradation of time guarantees by the platform, as reported in Section
...
presented for Linux$^{\mathbf{Xen}}$, since both platforms use Adeos nanokernel.
Some of the results presented in this paper have recently been discussed in a local 
forum \cite{Regnier08-wso}.
...
%Luciano
% In this work, we have conducted a timing evaluation of two Linux-based real-time
% OS. We have performed experiments to obtain comparative measures of both interrupt
% and activation latencies in different load scenarios. Both processor and external
% events generated by interrupts have been considered.

In this work, we have conducted a comparative evaluation of two Linux-based
real-time operating systems. Our comparative methodology has allowed experimental measurements of interrupt and
activation latencies in scenarios of variable load. Loads of both processing and 
those due to interrupt handling have been considered. Two types of experiments have been defined. 
In the simpler one, the same station that deals with real-time activities is responsible for the measurements. In the second, the measurements are carried out externally, by a different station. Both experiments can be used for comparison purposes although the second one gives the values of interrupt latencies more accurately. 


%Luciano
% While the standard Linux kernel have presented worst-case latencies over $100 \mu
% s$, Linux$^{\mathbf{Prt}}$ and Linux$^{\mathbf{Xen}}$ have shown to provide temporal
% guarantees within $20 \mu s$. However, in order to achieve this behavior in
% Linux$^{\mathbf{Prt}}$, interrupt threading for the parallel port handler had to
% disabled. As turns option turns the OS less flexible (POR QUE MENOS FLEXIVEL
% ?). Overall, Linux$^{\mathbf{Xen}}$ showed better results in achieving the temporal
