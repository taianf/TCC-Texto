...
\author{Paul D. E. Regnier \inst{1}
  \hspace{0.2cm} George M. Lima \inst{1}
  \email{\{gmlima, pregnier\}@ufba.br}
...
\begin{abstract}
    ...
    Ethernet hardware, can implements a decentralized medium
...
be presented in details in the next few sections. We will
...
messages will not be described. Important mechanisms like
...
We consider a set of nodes connected through a
shared-Ethernet segment, called the \doris{} segment.
...
The communication on a \doris{} segment is structured as a
...
We define a \doris{} \emph{rotating period} $RP_k$ associated
to $DS_k$ as the ordered list of $N_H$ consecutive \doris{} segment
that begins by $DS_k$, ie $RP_k = (DS_k, \dotsc , DS_{k+N_H-1})$
...
\begin{figure}[ht]
  ...
  \caption{The $\mathbf{N_H}$ \doris{} \emph{segments} of rotating
           period $\mathbf{RP_1}$ with a focus on $\mathbf{DS_i}$
  \label{fig:dorisStruct}}
\end{figure}

We assume that tasks and processes are equipped with local timers,
respectively denoted $t_{T_i}$ and $t_{P_i}$, instances of a node timer.
...
assumed to be tolerated. we are currently dealing with this aspects
...
Two others interesting properties must be highlighted.
...
$K_{P_i}$ may be out of date when $P_i$ becomes correct again.
...
The described protocol is currently being formally specified and then will be
...
referees for their valuable advises and comments. We
